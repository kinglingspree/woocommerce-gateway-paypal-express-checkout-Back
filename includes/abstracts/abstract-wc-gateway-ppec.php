...
abstract class WC_Gateway_PPEC extends WC_Payment_Gateway {
	...
	protected function validate_active_credentials() {
		...
		if ( ! $has_client_id ) {
			$errors[] = __( 'Error: You must enter Client ID for REST API.', 'woocommerce-gateway-paypal-express-checkout' );
		}
		
		if ( ! $has_client_secret ) {
			$errors[] = __( 'Error: You must enter Client Secret for REST API.', 'woocommerce-gateway-paypal-express-checkout' );
		}
		...
		if ( $has_client_id && $has_client_secret ) {
			try {
				...
				if ( ! $result ) {
					$errors[] = __( 'Error: The REST API credentials you provided are not valid. Please double-check that you entered them correctly and try again.', 'woocommerce-gateway-paypal-express-checkout' );
				}
			} catch ( Exception $ex ) {
				$errors[] = __( 'An error occurred while trying to validate your REST API credentials. Unable to verify that your API credentials are correct.', 'woocommerce-gateway-paypal-express-checkout' );
			}
		}
